<script>
        ...
        // =================================================================
        // UPDATED FUNCTION: Collects data and formats it as a simple string
        // =================================================================
        function collectAndStoreFamilyData() {
            ...
            document.querySelectorAll('.member-block').forEach(block => {
                ...
                if (name && age) {
                    let memberString = `${name} (${age})`; // Start with "Name (Age)"

                    // Check if ID field is visible (age >= 18) and has a value
                    if (idInput && !idInput.classList.contains('hidden') && id) {
                        // Check ID validation before adding it (though browser handles primary validation)
        ...
        function initializeMap(latitude, longitude) {
            // Set default view to coordinates or a fallback center
            const center = [latitude || 20.5937, longitude || 78.9629]; // Center of India

            if (!isMapInitialized) {
                map = L.map('map').setView(center, 5); // Default zoom 5
                ...
                map.setView(center, 13); // Zoom closer if already initialized
